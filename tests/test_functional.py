...
def test_annual_balance_calculation():
    ...
    annual_balance = manager.get_annual_balance(2025)
    assert (
        annual_balance == 1490.0
    )  # 7500.0 in transfers minus 910.0 in bills minus new bills 500.0 and 4500.0
...
def test_tenant_blacklist_check():
    ...
    manager.tenants_blacklist.append(
        TenantBlacklistEntry(tenant="Jan Pawlak", reason="Previous unpaid rent")
    )
